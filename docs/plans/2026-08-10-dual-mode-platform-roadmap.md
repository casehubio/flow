...
### Task 2.4: MCP infrastructure + generator

**Repo:** casehub-platform
**Files:**
- Create: `api/src/main/java/io/casehub/platform/api/mcp/ModelProvider.java` — SPI interface (pure Java, Tier 1)
- Create: `api/src/main/java/io/casehub/platform/api/mcp/OperationDescriptor.java` — operation metadata record
- Create: `mcp/pom.xml` — new submodule `casehub-platform-mcp` (runtime, hand-written, fixed)
- Create: `mcp/src/main/java/io/casehub/platform/mcp/CaseHubMcpTools.java` — `@McpTool` beans
- Modify: `generator-deployment/` — add MCP ModelProvider generation to the processor

**Architecture:**

```
casehub-platform-api (Tier 1, pure Java)
  └── ModelProvider SPI — domain(), operations(), summary()
  └── OperationDescriptor — name, type, params, returnType, description

casehub-platform-mcp (runtime module, hand-written, fixed)
  └── quarkus-mcp-server-sse dependency
  └── CaseHubMcpTools (@McpServer("casehub"))
        casehub_model(path?)  → CDI Instance<ModelProvider>, aggregates by domain
        casehub_action(domain, op, params) → dispatches to service method

Per-domain (generated by build-time extension):
  └── EngineModelProvider implements ModelProvider  (generated)
  └── WorkModelProvider implements ModelProvider    (generated)
```

**Every app that includes `casehub-platform-mcp` gets an MCP endpoint.** The model tree reflects only what's deployed in that app — CDI discovers only the ModelProviders on that app's classpath. AML sees engine+work+ledger+aml. Scaffold sees everything.

**MCP tool set (fixed, hosted by `quarkus-mcp-server-sse`):**
- `casehub_model(path?)` — two-hop hierarchy. No path = domain list with summaries + operation counts. Path = `{domain}` = all operations with parameter details.
- `casehub_action(domain, operation, params)` — dispatches to the service method via provider registry

**Generation (per `@PlatformService` domain):**
- Generate `ModelProvider` implementation that lists operations under domain X
...
- Generation counter: monotonic counter incremented when providers register/deregister

- [ ] Create `ModelProvider` SPI and `OperationDescriptor` record in platform-api
- [ ] Create `casehub-platform-mcp` module with `quarkus-mcp-server-sse` dependency
- [ ] Implement `CaseHubMcpTools` — `casehub_model` aggregates ModelProviders, `casehub_action` dispatches
- [ ] Add ModelProvider generation to the build-step processor
- [ ] Write tests: annotated interface → generated ModelProvider → model returns operations → action dispatches correctly
- [ ] Commit: `feat(platform): MCP infrastructure + model provider generation`
...
**Repo:** scaffold
**Files:**
- Modify: `pom.xml` — add dependencies: generator runtime, all foundation api modules, `casehub-platform-mcp`
...
- `casehub-platform-mcp` auto-discovers all generated ModelProviders → MCP endpoint works
- Verify composed GraphQL schema via introspection (no type name collisions)
- Verify `casehub_model()` returns all domains with correct operation counts
- Configure GraphQL UI for dev mode
- Configure MCP SSE endpoint
- Integration tests: full lifecycle via each presentation layer

- [ ] Add generator + foundation api + platform-mcp dependencies
- [ ] Configure GraphQL endpoint in application.properties
- [ ] Configure MCP SSE endpoint in application.properties
- [ ] Integration test: startCase via REST, query via GraphQL, discover via MCP model, execute via MCP action
...
- [ ] Verify `casehub_action("engine", "startCase", {...})` works
- [ ] Commit: `feat(scaffold): unified CaseHub server — generated REST + GraphQL + MCP`

### Task 3.6: Retire hand-written REST and per-module MCP tools

**Repos:** casehub-engine, casehub-work, casehub-ledger, casehub-qhorus

**Prerequisite:** Generated REST validated against hand-written REST in Tasks 3.1–3.4. MCP model verified in Task 3.5.

**REST modules to retire:**
- [ ] Remove `casehub-engine-rest` module
- [ ] Remove `casehub-work-rest` module
- [ ] Remove `casehub-ledger-rest` module
- [ ] Remove qhorus hand-written REST endpoints (in runtime/)

**MCP modules to retire (replaced by hierarchical model):**
- [ ] Remove `casehub-engine-mcp` module (flat tool list → replaced by generated ModelProvider)
- [ ] Remove per-module `@McpTool` classes in qhorus (replaced by generated ModelProvider)

- [ ] Update all downstream consumers that depended on retired modules
- [ ] Run full platform build
- [ ] Commit per repo: `refactor(<repo>): retire hand-written REST + MCP — replaced by generated`
